describe('Keypad.Control', function () {
  ...
  describe('keypad public methods', function () {
    ...
    beforeEach(inject(function(){
      ...
      $scope.$parent = { amount: '500', message: {}, withdrawlpriortiy: 'least'};
      $scope.locale = 'en-gb';
    ...
    it('withdraw(): throws error if withdrawl amount cant be made up from available float', function() {
      ...
      $scope.$parent.withdrawlpriortiy = 'least';
      $scope.currentbalance = 465;
      $scope.float = [ {'denomination': 1, 'amount': 5}, {'denomination': 20, 'amount': 3}, { 'denomination': 100, 'amount': 4} ];
      $scope.$parent.float = $scope.float;

      expect(function() { $scope.withdraw(446); }).toThrow(new Error('Sorry - we cant provide that withdrawal amount. The float is 1 short'));
    ...
    it('withdraw(): when requested returns an object using the most possible priorty denomination units', function() {
      ...
      $scope.$parent.withdrawlpriortiy = 'denomination';
    ...
    it('toggle(): switching to "least" mode', function() {
      ...
      expect($scope.toggle( false )).toBe('least');
    });
